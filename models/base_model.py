...
class BaseModel:
    ...
    def to_dict(self) -> dict:
        ...
        # if k not in Other_instances}
        data = {}
        # Initialize an empty dictionary to store the extracted data
        # Iterate over each key-value pair in the object's __dict__
        for new_key, new_value in self.__dict__.items():
            # Check if the key is not in the Other_instances list
            if new_key not in new_instances:
                # Add the key-value pair to the data dictionary
                # if the key is not in Other_instances
